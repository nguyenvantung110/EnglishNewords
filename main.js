...
function displayWords(words) {
    words.forEach((word) => {
        ...
        wordItem.innerHTML = `
                ...
                <td>${word[1]}</td>
                <td>${word[2]}</td>
                <td onclick="speakWord('${word[0]}')">${SPEAKER_SVG}</td>
...
function speakWord(word){
     // Check browser support Web Speech API or not
  if ('speechSynthesis' in window) {
    // Create object SpeechSynthesisUtterance
    let utterance = new SpeechSynthesisUtterance(word);
    
    // Tùy chọn: Đặt ngôn ngữ (ví dụ: tiếng Việt)
    utterance.lang = 'en-US';
    
    // Phát âm từ
    window.speechSynthesis.speak(utterance);
  } else {
    console.log("Browser not support Web Speech API");
  }
}
